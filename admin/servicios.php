...
if($_POST && isset($_POST['accion']) && $_POST['accion'] === 'agregar'){
    ...
    $conexion->query("INSERT INTO servicios (nombre,descripcion,precio) VALUES ('$nombre','$desc',$precio)");
    $msg = 'agregado';
}
...
if($_POST && isset($_POST['accion']) && $_POST['accion'] === 'editar'){
    ...
    $conexion->query("UPDATE servicios SET nombre='$nombre',descripcion='$desc',precio=$precio,activo=$activo WHERE id=$id");
    $msg = 'editado';
}

if(isset($_GET['borrar'])){
    $conexion->query("DELETE FROM servicios WHERE id=".intval($_GET['borrar']));
    header("Location: servicios.php?msg=borrado"); exit;
}
?>
<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
<title>Servicios — Admin Ruta 20</title>
...
<body>
<div class="topbar">
    <div class="topbar-brand">RUTA <span>20</span> <span style="font-weight:300;font-size:0.85rem;color:rgba(200,214,248,0.4)">/ Servicios</span></div>
    <a href="dashboard.php">← Dashboard</a>
</div>
<div class="wrap">
    <div class="page-title">Servicios Adicionales</div>
    <div class="page-sub">Gestiona tiquetes, seguros, guías y otros servicios con sus tarifas</div>

    <?php if($msg): ?>
    <div class="alert-ok">✓ Servicio <?php echo $msg; ?> correctamente</div>
    <?php endif; ?>
    <?php if(isset($_GET['msg'])): ?>
    <div class="alert-ok">✓ Servicio eliminado</div>
    <?php endif; ?>
        ...
        <form method="POST">
            ...
            <div class="row">
                <div class="col-md-5 form-group">
                    <label class="adm-label">Nombre del servicio</label>
                    <input type="text" name="nombre" class="form-control" placeholder="Ej: Tiquete aéreo ida y vuelta" required>
                </div>
                <div class="col-md-4 form-group">
                    <label class="adm-label">Descripción</label>
                    <input type="text" name="descripcion" class="form-control" placeholder="Descripción breve">
                </div>
                <div class="col-md-3 form-group">
                    <label class="adm-label">Precio</label>
                    <input type="number" name="precio" class="form-control" placeholder="450000" required>
                </div>
            </div>
            <button type="submit" class="btn-save">Agregar servicio</button>
        </form>
    </div>
    ...
    <div class="card-admin">
        ...
        <table class="table table-bordered table-striped">
            <thead>
                <tr><th>Servicio</th><th>Descripción</th><th>Precio</th><th>Estado</th><th>Acciones</th></tr>
            </thead>
            <tbody>
            <?php
            $servs = $conexion->query("SELECT * FROM servicios ORDER BY nombre");
            while($s = $servs->fetch_assoc()):
            ?>
            <tr>
                <td style="font-weight:500"><?php echo htmlspecialchars($s['nombre']); ?></td>
                <td style="color:rgba(200,214,248,0.5);font-size:0.8rem"><?php echo htmlspecialchars($s['descripcion']); ?></td>
                <td style="color:var(--gold2);font-weight:600;">$<?php echo number_format($s['precio'],0,',','.'); ?></td>
                <td><span class="<?php echo $s['activo']?'badge-on':'badge-off'; ?>"><?php echo $s['activo']?'Activo':'Inactivo'; ?></span></td>
                <td>
                    <button class="btn-edit-sm" onclick="abrirEditar(<?php echo htmlspecialchars(json_encode($s)); ?>)">Editar</button>
                    <a href="servicios.php?borrar=<?php echo $s['id']; ?>" class="btn-del-sm"
                       onclick="return confirm('¿Eliminar servicio?')">Borrar</a>
                </td>
            </tr>
            <?php endwhile; ?>
            </tbody>
  ...
  <div class="modal-dialog">
    <div class="modal-content p-2">
      <div class="modal-header">
        <h5 class="modal-title">✏️ Editar servicio</h5>
...
function abrirEditar(s) {
    ...
    document.getElementById('edit-activo').checked = s.activo == 1;
