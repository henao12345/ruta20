...
if($_POST && isset($_POST['accion']) && $_POST['accion'] === 'agregar'){
    ...
    $conexion->query("INSERT INTO actividades (nombre,ciudad,tipo,descripcion,precio) VALUES ('$nombre','$ciudad','$tipo','$desc',$precio)");
    $msg = 'agregado';
}
...
if($_POST && isset($_POST['accion']) && $_POST['accion'] === 'editar'){
    ...
    $conexion->query("UPDATE actividades SET nombre='$nombre',ciudad='$ciudad',tipo='$tipo',descripcion='$desc',precio=$precio,activo=$activo WHERE id=$id");
    $msg = 'editado';
}

if(isset($_GET['borrar'])){
    $conexion->query("DELETE FROM actividades WHERE id=".intval($_GET['borrar']));
    header("Location: actividades.php?msg=borrado"); exit;
}
?>
<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
<title>Actividades — Admin Ruta 20</title>
...
.badge-tipo-act{ background:rgba(34,197,94,0.15);color:#86efac;padding:3px 10px;border-radius:20px;font-size:0.7rem; }
.badge-tipo-tras{ background:rgba(59,130,246,0.15);color:#93c5fd;padding:3px 10px;border-radius:20px;font-size:0.7rem; }
...
<body>
<div class="topbar">
    <div class="topbar-brand">RUTA <span>20</span> <span style="font-weight:300;font-size:0.85rem;color:rgba(200,214,248,0.4)">/ Actividades</span></div>
    <a href="dashboard.php">← Dashboard</a>
</div>
<div class="wrap">
    <div class="page-title">Actividades y Traslados</div>
    <div class="page-sub">Gestiona tours, actividades y traslados con sus precios por ciudad</div>

    <?php if($msg): ?>
    <div class="alert-ok">✓ Registro <?php echo $msg; ?> correctamente</div>
    <?php endif; ?>
    <?php if(isset($_GET['msg'])): ?>
    <div class="alert-ok">✓ Registro eliminado</div>
    <?php endif; ?>

    <div class="card-admin">
        <h3>➕ Agregar actividad o traslado</h3>
        <form method="POST">
            <input type="hidden" name="accion" value="agregar">
            <div class="row">
                <div class="col-md-4 form-group">
                    <label class="adm-label">Nombre</label>
                    <input type="text" name="nombre" class="form-control" placeholder="Ej: Termales Santa Rosa" required>
                </div>
                ...
                <div class="col-md-2 form-group">
                    <label class="adm-label">Precio / persona</label>
                    <input type="number" name="precio" class="form-control" placeholder="80000" required>
                </div>
                <div class="col-md-2 form-group">
    ...
    <div class="card-admin">
        <h3>🎯 Actividades y traslados registrados</h3>
        <div style="overflow-x:auto;">
        <table class="table table-bordered table-striped">
            <thead>
                <tr><th>Nombre</th><th>Ciudad</th><th>Tipo</th><th>Precio</th><th>Descripción</th><th>Estado</th><th>Acciones</th></tr>
            </thead>
            <tbody>
            <?php
            $acts = $conexion->query("SELECT * FROM actividades ORDER BY ciudad, tipo, nombre");
            while($a = $acts->fetch_assoc()):
            ?>
            <tr>
                <td style="font-weight:500"><?php echo htmlspecialchars($a['nombre']); ?></td>
                <td><span class="badge-ciudad badge-<?php echo strtolower($a['ciudad']); ?>"><?php echo $a['ciudad']; ?></span></td>
                <td><span class="badge-tipo-<?php echo $a['tipo']==='traslado'?'tras':'act'; ?>"><?php echo ucfirst($a['tipo']); ?></span></td>
                <td style="color:var(--gold2);font-weight:600;">$<?php echo number_format($a['precio'],0,',','.'); ?></td>
                <td style="color:rgba(200,214,248,0.5);font-size:0.78rem"><?php echo htmlspecialchars($a['descripcion']); ?></td>
                <td><span class="<?php echo $a['activo']?'badge-on':'badge-off'; ?>"><?php echo $a['activo']?'Activo':'Inactivo'; ?></span></td>
                <td>
                    <button class="btn-edit-sm" onclick="abrirEditar(<?php echo htmlspecialchars(json_encode($a)); ?>)">Editar</button>
                    <a href="actividades.php?borrar=<?php echo $a['id']; ?>" class="btn-del-sm"
  ...
  <div class="modal-dialog">
    <div class="modal-content p-2">
      <div class="modal-header">
        <h5 class="modal-title">✏️ Editar actividad</h5>
...
function abrirEditar(a) {
    ...
    document.getElementById('edit-ciudad').value = a.ciudad;
    document.getElementById('edit-tipo').value   = a.tipo;
    document.getElementById('edit-precio').value = a.precio;
    document.getElementById('edit-desc').value   = a.descripcion || '';
    document.getElementById('edit-activo').checked = a.activo == 1;
